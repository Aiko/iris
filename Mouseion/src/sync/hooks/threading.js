...
//? Phase 1: Exhaustively finds all previous messages in a thread using reply-to and references.
//? Phase 2: Combines threads that meet certain requirements.
//? Phase 3: Determines the "Aiko Folder" (aka board or inbox), and ensures every message is present in that.

module.exports = (Registry, actually_thread=true) => {

  //? Retrieve necessary modules from registry
  const { user, provider } = Registry.get('imap-config')
  const Configuration = Registry.get('Configuration')
  const CacheDB = Registry.get('Cache')
  const Courier = Registry.get('Courier')
  const Contacts = Registry.get('Contacts')
  const BoardRules = Registry.get('Board Rules')
  ...
  //? Initialize a Log
  const Log = Lumberjack('Threading')

  //? Phase 2 and 3 queues
  const p2_queue = []
  const p3_queue = []

  //? "email" is a Janitor-cleaned header-level or higher parsed envelope
  const Phase1 = async (email, cursor) => {

    //? Assert header-level parsing
    if (!(email.M.references)) {
      Log.error(`Attempted to thread <mid:${email.M.envelope.mid}>, but the envelope does not meet the minimum of header-level parsing.`)
      return null
    }

    //? If we already have the message locally we can skip Phase 1
    const exists = await CacheDB.lookup.mid(email.M.envelope.mid)
    if (exists) {
      //? Ensure the message is in the parsed location
      await CacheDB.add.message(email.M.envelope.mid, email.folder, email.uid, email.M.envelope.cleanSubject, cursor, {
        timestamp: email.M.envelope.date
      })
      //? Queue it for Phase 2
      p2_queue.push(exists.tid)
      return exists.tid
    }
