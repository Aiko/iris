...
export default class Mailbox {
  ...
  async run() {
    ...
    this.lock.acquire(async () => {
      ...
      if (this.ran) return this.Log.warn("Already ran 10TEC challenge, refusing to go agane.");
      ...
      try {
        ...
        const syncStatus = await _this.sync.sync10TEC(this.folders.archive() ?? "INBOX")
        if (!syncStatus) {
