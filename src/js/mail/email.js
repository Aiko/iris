...
ipcMain.handle('please get emails', async (_, q) => {
  ...
  if (!messages/* || messages.length===0 */) { return { error: `Did not receive any messages back when calling client.listMessages(${path}, ${sequence}, ${peek}) in "please get emails"` } }
...
ipcMain.handle('please set email flags', async (_, q) => {
  ...
  if (!blind && (!messages || messages.length === 0)) { return { error: 'Did not receive any messages back when calling client.setFlags in "please set email flags"' } }
