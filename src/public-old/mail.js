  ...
  methods: {
    ...
    async syncWithMailServer () {
      ...
      this.mailbox.boards = await Promise.all(this.mailbox.boards.map(async board => {
        ...
        if (board_folders.includes(board.folder)) { return board }
    ...
    async processEmails (emails) {
      return await Promise.all(emails.map(async email => {
        ...
        if (!email.verify && email.headers && email.headers['list-unsubscribe']) { email.unsubscribe = email.headers['list-unsubscribe'].replace(/<|>/g, '') }
      ...
      try {
        ...
        senders.map(email => {
          if (email.from && email.from.length > 0) {
            ...
            if (from.address) {
              if (!contacts[from.address]) { contacts[from.address] = from.name || from.address }
            }
          }
        })
    ...
    async fetchEmails (start, stop, overwrite = true, sort = false, filterDups = true, getBoards = true, pushToEnd = false, fetchAnyways = false) {
      ...
      try {
        ...
        if (emails.length > 0) {
          ...
          if (pushToEnd) { this.emails.push(...emails) } else { this.emails.unshift(...emails) }
    ...
    async setActiveEmail (email) {
      ...
      if (email.summary) {
        ...
        while (match = ptn.exec(email.summary)) {
          if (email.boldedHtml.includes(match[0]) && email.boldedHtml.indexOf(match[0]) > -1) { email.boldedHtml = email.boldedHtml.slice(0, email.boldedHtml.indexOf(match[0])) + '<b title="Detected as most important sentence of the email." style="color: #fff;padding: 1px 0px 1px 0px;border-radius: 3px;display: inline;background: #4b74ff;box-shadow: 2px 0 0 #4b74ff, -3px 0 0 #4b74ff;cursor:pointer;">' + match[0] + '</b>' + email.boldedHtml.slice(email.boldedHtml.indexOf(match[0]) + match[0].length) }
        }
      }
