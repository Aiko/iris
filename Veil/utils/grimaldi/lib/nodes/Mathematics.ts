...
export const Mathematics = Node.create<MathOptions>({
  ...
  addAttributes() {
    return {
      ...
      src: {
        ...
        renderHTML: attrs => ({
          src: attrs.src || ("https://math.now.sh/?color=red&from=" + (attrs.formula ?? "?"))
        })
      },
      id: {
  ...
  renderHTML({ HTMLAttributes, node }) {
    ...
    const url = "https://math.now.sh/?color=red&from=" + (node.attrs.formula ?? "?")
    window.downloadAndFillImage(url, id)
